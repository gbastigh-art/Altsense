...
static void heart_tick(level_t *level, entity_t *ent) {
...
static void heart_fixed_update(level_t *l, entity_t *e, f32 dt) {
    entity_face_dir(l, e, e->fly_dir, 0.9f, dt);
    enemy_fly_towards(l, e, 90.0f, e->fly_dir, dt);
}

static void heart_move_on_hit(
        level_t *l,
        entity_t *e,
...
static void heart_on_move_portal(
        level_t *level,
        entity_t *ent,
        const trace_hit_t *hit) {
...
static void heart_models(
    ...
    tr = m4_mul(tr, m4_rotate_make(-clamp(e->dir.z, -0.15f, 0.15f), v3_of(0, 1, 0)));
    tr = m4_mul(tr, m4_scale_make(v3_of(1.5f)));
    ...
    *dynlist_push(*models) =
        (model_t) {
            .id = lptr_from(e),
            .data = model_atlas_lookupf("unit_cube"), // TODO
...
ENTITY_TYPE_REGISTER(
    ...
    (entity_type_t) {
        ...
        .fixed_update_fn = heart_fixed_update,
        .tick_fn = heart_tick,
        .move_on_hit_fn = heart_move_on_hit,
        .on_move_portal_fn = heart_on_move_portal,
        .on_hit_fn = heart_on_hit,
