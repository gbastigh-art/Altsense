...
void *_llist_pop_front_impl(void *_list, int offset);
...
// remove _n from list
#define llist_remove(_field, _plist, _n) ({                  \
        CHECK_FIELD(typeof(*(_plist)), head);                \
        typeof(_n) __n = (_n);                               \
        ((_llist_func_impl)(_llist_remove_impl))(            \
            (_plist), __n, offsetof(typeof(*(_n)), _field)); \
        __n;                                                 \
    })
...
#define llist_pop_front(_field, _plist)                                      \
    _llist_pop_front_impl((_plist), offsetof(typeof(*(_plist)->head), _field))
...
void *_llist_pop_front_impl(void *_list, int offset) {
    LLIST(void) *list = _list;
    void *head = list->head;
    LLIST_NODE(void) *node_head = (void*) (((u8*) head) + offset);
    list->head = node_head->next;
    node_head->next = NULL;
    return head;
}
