...
#define ENUM_INI_ERROR(F, ...)             \
    F(OK,                  0, __VA_ARGS__) \
    F(UNTERMINATED_STRING, 1, __VA_ARGS__) \
    F(BAD_SECTION,         2, __VA_ARGS__) \
    F(BAD_PROP,            3, __VA_ARGS__) \
    F(BAD_ESCAPE,          4, __VA_ARGS__) \
    F(FILE_ERROR,          5, __VA_ARGS__) \

ENUM_DECL(ini_error, INI, ENUM_INI_ERROR)
...
ENUM_DEFINE(ini_error, INI_ERROR, ENUM_INI_ERROR)
...
static void free_prop(map_t *m, void *p) {
    prop_t *prop = p;
...
static section_t *create_section(ini_t *ini, const char *name, int id) {
    ...
    section->id = id;
    section->name = mem_strdup(ini->allocator, name);
...
static prop_t *get_or_create_prop(
    ...
    const char *name_copy = mem_strdup(ini->allocator, name);
...
ini_error_e ini_init_from_path(
    ...
    const file_error_e ferr = file_read_strbuf(&buf, path);
    if (ferr != FILE_OK) {
    ...
    const ini_error_e err = ini_parse(ini, buf);
    if (err != INI_OK) {
        ini_destroy(ini);
    }
...
#define COMMENT_CHARS "#;"
    ...
    while (*next && (next = str_line(next, line, sizeof(line)))) {
        ...
        if (*p == '[') {
            ...
            const str_view_t view = str_view_from(buf);
            str_view_t view_unescaped = str_view_unescape(&view, tlscratch());

            if (!str_view_valid(&view_unescaped)) {
                err = INI_BAD_ESCAPE;
                goto done;
            }

            view_unescaped = str_view_trim(&view_unescaped);

            // add property
            prop_t *prop = get_or_create_prop(ini, section, name);
            prop->value = str_view_dump(&view_unescaped, ini->allocator);
        }
    }
...
ini_error_e ini_parse_from_path(ini_t *ini, const char *path) {
    ...
    const file_error_e ferr = file_read_strbuf(&buf, path);
    if (ferr != FILE_OK) {
        WARN("file error: %s", file_error_to_str(ferr));
        return INI_FILE_ERROR;
    }

    return ini_parse(ini, buf);
}
...
void ini_dump_to_file(ini_t *ini, FILE *fp) {
    ...
    map_each(char*, prop_t, &global->props, it) {
        const str_view_t view = str_view_from(it.value->value);
        fprintf(
            fp,
            "%s = %s\n",
            *it.key,
            str_view_escape(&view, tlscratch()).start);
    }
    ...
    map_each(int, section_t, &ini->sections, it_s) {
        ...
        map_each(char*, prop_t, &it_s.value->props, it_p) {
            const str_view_t view = str_view_from(it_p.value->value);
            fprintf(
                fp,
                "%s = %s\n",
                *it_p.key,
...
ini_error_e ini_dump_to_path(ini_t *ini, const char *path) {
    char *str = ini_dump_to_str(ini, tlscratch());

    const str_view_t view = str_view_from(str);
    const file_error_e ferr = file_write_str_view(path, &view);
    if (ferr != FILE_OK) {
        WARN("ini file error: %s", file_error_to_str(ferr));
        return INI_FILE_ERROR;
    }
...
void ini_remove(ini_t *ini, const char *section, const char *prop) {
    ...
    section_t *s = get_section_by_name(ini, section);
    ASSERT_DEBUG(section);

    prop_t p;
    map_try_remove(&s->props, prop, &p);
    ...
    if (s->id != GLOBAL_SECTION_ID && map_empty(&s->props)) {
        ...
        map_destroy(&s->props);
        ASSERT(map_try_remove(&ini->sections, s->id));
    }
}
...
void ini_set(
    ...
    p->value = mem_strdup(ini->allocator, value);
}
...
void ini_set_fmt(
    ...
    va_list ap;
    va_start(ap, fmt);
    ini_set(ini, section, prop, mem_vstrfmt(tlscratch(), fmt, ap));
    va_end(ap);
}
...
bool ini_get_bool(
    ...
    for (int i = 0; i < ARRLEN(true_matches); i++) {
        if (!strcmp(p, true_matches[i])) { *out = true; return true; }
    }

    for (int i = 0; i < ARRLEN(false_matches); i++) {
