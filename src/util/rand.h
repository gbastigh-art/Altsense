...
// random v3 "conical" to n within theta
v3 rand_v3_cone(rand_t *prand, v3 n, f32 theta);

// random v3 "conical" to n within theta_min..theta_max
v3 rand_v3_cone_slice(rand_t *prand, v3 n, f32 theta_min, f32 theta_max);
...
v2 rand_v2_dir(rand_t *prand) {
    ...
    return v2_of(fast_cos(a), fast_sin(a));
}
...
v3 rand_v3_cone(rand_t *prand, v3 n, f32 theta) {
    ...
    const f32 z = rand_f32(prand, fast_cos(theta), 1.0f);
    const f32 q = sqrtf(1.0f - (z * z));
    const v3 v = v3_of(q * fast_cos(phi), q * fast_sin(phi), -z);

    n = v3_normalize(n);

    // ensure n is not parallel with p
    v3 p = v3_of(0, 0, 1);
    if (fabsf(fabsf(v3_dot(n, p)) - 1.0f) < 0.000001f) {
        p = v3_of(0, 1, 0);
    }

    const m4 m = cam_lookat(v3_of(0), n, p);
    return v3_normalize_from(m4_mulv(m4_inv(m), v4_of(v, 0.0)));
}

// rand_v3_cone, but with theta_max so we get a slice of a cone
// theta_min and theta_max go "away" from n on the cone, so we must enforce
// theta_max > theta_min
v3 rand_v3_cone_slice(rand_t *prand, v3 n, f32 theta_min, f32 theta_max) {
    ASSERT_DEBUG(theta_max >= theta_min);
    if (theta_max < 0.000001f) { return n; }

    const f32 phi = rand_f32(prand, 0.0f, TAU);
    const f32 z = rand_f32(prand, fast_cos(theta_max), fast_cos(theta_min));
    const f32 q = sqrtf(1.0f - (z * z));
    const v3 v = v3_of(q * fast_cos(phi), q * fast_sin(phi), -z);
...
v2 rand_v2_cone(rand_t *prand, v2 n, f32 theta) {
    const f32 a = rand_f32(prand, -theta, theta);
    return v2_rotate(n, a);
}
