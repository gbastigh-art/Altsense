...
// configure depth, but our proj matrices are left handed on all platforms :)
#ifdef SOKOL_METAL
    // depth is 0..1
...
#include "util/math/util.h"       /* IWYU pragma: export */

#include "util/math/aliases.h"    /* IWYU pragma: export */

#define MKV2I2(_x, _y) ((v2i) {{ (_x), (_y) }})
#define MKV2I1(_s) ({ typeof(_s) __s = (_s); ((v2i) {{ __s, __s }}); })
#define MKV2I0() ((v2i) {{ 0, 0 }})
#define v2i_of(...) (VMACRO(MKV2I, __VA_ARGS__))

#define MKV2I_CONST2(_x, _y) (((v2i) {{ (_x), (_y) }}))
#define MKV2I_CONST1(_s) ((v2i) {{ _s, _s }})
#define MKV2I_CONST0() ((v2i) {{ 0, 0 }})
#define v2i_const(...) VMACRO(MKV2I_CONST, __VA_ARGS__)

#define MKV3I3(_x, _y, _z) ((v3i) {{ (_x), (_y), (_z) }})
#define MKV3I1(_s) ({ typeof(_s) __s = (_s); ((v3i) {{ __s, __s, __s }}); })
...
#define MKV4I1(_s) ({ typeof(_s) __s = (_s); ((v4i) {{ __s, __s, __s, __s }}); })
#define MKV4I0() ((v4i) {{ 0, 0 }})
#define v4i_of(...) VMACRO(MKV4I, __VA_ARGS__)

#define MKV22(_x, _y) ((v2) {{ (_x), (_y) }})
#define MKV21(_s) ({ typeof(_s) __s = (_s); ((v2) {{ __s, __s }}); })
#define MKV20() ((v2) {{ 0, 0 }})
#define v2_of(...) VMACRO(MKV2, __VA_ARGS__)
#define v2_normalize_of(...) (v2_normalize(v2_of(__VA_ARGS__)))
...
#define MKV32(_v2, _z) ({ typeof(_v2) _v = (_v2); ((v3) {{ _v.x, _v.y, (_z) }}); })
#define MKV31(_s) ({ typeof(_s) __s = (_s); ((v3) {{ __s, __s, __s }}); })
#define MKV30() ((v3) {{ 0, 0 }})
#define v3_of(...) VMACRO(MKV3, __VA_ARGS__)
#define v3_normalize_of(...) (v3_normalize(v3_of(__VA_ARGS__)))
...
#define MKV41(_s) ({ typeof(_s) __s = (_s); ((v4) {{ __s, __s, __s, __s }}); })
#define MKV40() ((v4) {{ 0, 0 }})
...
#define V2_AXIS_X (v2_const(1, 0))
#define V2_AXIS_Y (v2_const(0, 1))

#define V3_AXIS_X (v3_const(1, 0, 0))
#define V3_AXIS_Y (v3_const(0, 1, 0))
#define V3_AXIS_Z (v3_const(0, 0, 1))
...
M_INLINE int v2i_min(v2i v) {
    return min(v.x, v.y);
}

M_INLINE int v2i_max(v2i v) {
    return max(v.x, v.y);
}

// M_INLINE v2i v2i_minv(v2i v, v2i u) {
//     return v2i_of(min(v.x, u.x), min(v.y, u.y));
// }
// 
// M_INLINE v2i v2i_maxv(v2i v, v2i u) {
...
// if v is not ok (!isnan && !isinf), then replace with u
M_INLINE v3 v3_valid_or(v3 v, v3 u) {
    return v3_isvalid(v) ? v : u;
}

// if v is not ok (!isnan && !isinf), then replace with u
M_INLINE v2 v2_valid_or(v2 v, v2 u) {
    return v2_isvalid(v) ? v : u;
}
...
// direction from one vector to another, or "otherwise" if invalid
M_INLINE v3 v3_dir_or(v3 from, v3 to, v3 otherwise) {
    const v3 v = v3_sub(to, from);
    const f32 d2 = v3_norm2(v);

    if (M_UNLIKELY(d2 < 0.000001f)) {
        return otherwise;
    } else {
        return v3_divs(v, sqrtf(d2));
    }
}

// v3_dir, but it gives (1, 0, 0) if the direction would be invalid
M_INLINE v3 v3_safedir(v3 from, v3 to) {
    return v3_dir_or(from, to, V3_AXIS_X);
}
...
// direction from one vector to another, or "otherwise" if invalid
M_INLINE v2 v2_dir_or(v2 from, v2 to, v2 otherwise) {
    const v2 v = v2_sub(to, from);
    const f32 d2 = v2_norm2(v);

    if (M_UNLIKELY(d2 < 0.000001f)) {
        return otherwise;
    } else {
        return v2_divs(v, sqrtf(d2));
    }
}

// v2_dir, but it gives (1, 0) if the direction would be invalid
M_INLINE v2 v2_safedir(v2 from, v2 to) {
    return v2_dir_or(from, to, V2_AXIS_X);
}
...
M_INLINE v2 v2_slerp(v2 from, v2 to, f32 t) {
    const f32
        dot = satf(v2_dot(from, to)),
        theta = acosf(dot) * t;
    const v2 rel = v2_normalize(v2_sub(to, v2_scale(from, dot)));
    return
        v2_normalize(
            v2_add(
                v2_scale(from, cosf(theta)),
                v2_scale(rel, sinf(theta))));
}
...
M_INLINE v3 v3_slerp(v3 from, v3 to, f32 t) {
    const f32
        dot = clamp(v3_dot(from, to), -1.0f, 1.0f),
        theta = acosf(dot) * t;
    const v3 rel = v3_normalize(v3_sub(to, v3_scale(from, dot)));
    return
        v3_normalize(
            v3_add(
                v3_scale(from, cosf(theta)),
                v3_scale(rel, sinf(theta))));
}
...
// v2_lerp but with unconstrained t
M_INLINE v2 v2_combine(v2 a, v2 b, f32 t) {
    const v2 diff = v2_sub(b, a);
    return v2_add(a, v2_scale(diff, t));
}

// v3_lerp but with unconstrained t
M_INLINE v3 v3_combine(v3 a, v3 b, f32 t) {
    const v3 diff = v3_sub(b, a);
    return v3_add(a, v3_scale(diff, t));
}
...
// get normalize x, y components of v
M_INLINE v2 v2_normalize_from(v3 v) {
    return v2_normalize(v2_from(v));
}

// get normalize x, y components of v
M_INLINE v3 v3_normalize_from(v4 v) {
    return v3_normalize(v3_from(v));
}

// "replace" v with w, having other axes follow suit
M_INLINE m4 m4_rotate_make_from_to(v3 v, v3 w) {
    v = v3_normalize(v);
    w = v3_normalize(w);

    const f32 dot = v3_dot(v, w);
    v3 axis = v3_normalize(v3_cross(v, w));
    f32 angle = acosf(dot);
    ...
    if (dot > 0.9999f) {
        ...
        // anti-parallel
        axis = v3_cross(v, v3_of(0, 1, 0));
        if (v3_norm2(axis) < 0.0001f) {
            axis = v3_cross(v, v3_of(1, 0, 0));
        }
        angle = PI;
    }

    return m4_rotate_make(angle, axis);
}
...
M_INLINE m4 m4_rotate_make_dir(v3 dir) {
    ...
    if (fabsf(fabsf(v3_dot(dir, v3_of(0, 0, 1))) - 1.0f) < 0.0001f) {
        y = v3_normalize(v3_cross(v3_of(0, 1, 0), dir));
        z = v3_normalize(v3_cross(dir, y));
    } else {
        z = v3_normalize(v3_cross(dir, v3_of(0, 0, 1)));
        y = v3_normalize(v3_cross(z, dir));
    }
    ...
    m.col[2] = v4_of(z, 0);
    // TODO: lol. lmao even.
    m = m4_mul(m, m4_rotate_make(-PI_2, v3_of(1, 0, 0)));
    return m;
}
...
M_INLINE v2 v2_floor(v2 v) {
    return v2_of(floorf(v.x), floorf(v.y));
}

M_INLINE v3 v3_floor(v3 v) {
    return v3_of(floorf(v.x), floorf(v.y), floorf(v.z));
}

M_INLINE v2 v2_ceil(v2 v) {
    return v2_of(ceilf(v.x), ceilf(v.y));
}

M_INLINE v3 v3_ceil(v3 v) {
    return v3_of(ceilf(v.x), ceilf(v.y), ceilf(v.z));
}
...
M_INLINE v2 v2_abs(v2 v) {
    return v2_of(fabsf(v.x), fabsf(v.y));
}
...
M_INLINE v2 plane_project_basis_2d(
    ...
    // this is not terribly effecient, since we always know that the
...
// compute plane from point p and normal n
M_INLINE v4 plane_from_point_normal(v3 p, v3 n) {
    // (n_x * p_x) + (n_y * p_y) + (n_z * p_z) = d
    // solve for d
    return v4_of(n, -v3_dot(p, n));
}
...
// v4 -> u32 (u8x4) -> f32
M_INLINE f32 v4_to_f32(v4 v) {
    return i32_bits_to_f32(v4_to_u8x4(v));
}

// f32 -> u32 (u8x4) -> v4
M_INLINE v4 f32_to_v4(f32 f) {
    return u8x4_to_v4(f32_bits_to_i32(f));
}

// pack v4 with components in range -1..1 into u32
M_INLINE u32 v4n_pack_to_u32(v4 v) {
    const i8 x8 = (i8) (v.x * 127.0f);
    const i8 y8 = (i8) (v.y * 127.0f);
    const i8 z8 = (i8) (v.z * 127.0f);
    const i8 w8 = (i8) (v.w * 127.0f);
    return 0u
        | (((u32) w8) << 24)
        | (((u32) z8) << 16)
        | (((u32) y8) <<  8)
...
// undoes v4n_pack_to_u32
M_INLINE v4 u32_unpack_to_v4n(u32 u) {
    const i8 x8 = (i8) ((u >>  0) & 0xFF);
    const i8 y8 = (i8) ((u >>  8) & 0xFF);
    const i8 z8 = (i8) ((u >> 16) & 0xFF);
    const i8 w8 = (i8) ((u >> 24) & 0xFF);
    return v4_of(x8 / 127.0f, y8 / 127.0f, z8 / 127.0f, w8 / 127.0f);
}
