...
// TODO: if this gives problems, fix by using type-safe unions of T and u32[N]
#define BYTEBUF_DECL_RW_4xN(T, name, N)                                        \
    M_INLINE int bytebuf_peek_##name(bytebuf_t *b, T *out) {                   \
        for (uint i = 0; i < N; i++) {                                         \
            bytebuf_peek_u32(b, (u32*) (&out->raw[i]));                        \
        }                                                                      \
        return 4 * N;                                                          \
    }                                                                          \
                                                                               \
    M_INLINE int bytebuf_read_##name(bytebuf_t *b, T *out) {                   \
        for (uint i = 0; i < N; i++) {                                         \
            bytebuf_read_u32(b, (u32*) (&out->raw[i]));                        \
        }                                                                      \
        return 4 * N;                                                          \
    }                                                                          \
                                                                               \
    M_INLINE int bytebuf_write_##name(bytebuf_t *b, T val) {                   \
        for (uint i = 0; i < N; i++) {                                         \
            bytebuf_write_u32(b, *(u32*) (&val.raw[i]));                       \
        }                                                                      \
        return 4 * N;                                                          \
    }
...
BYTEBUF_DECL_RW_4xN(v2,  v2,  2)
BYTEBUF_DECL_RW_4xN(v3,  v3,  3)
BYTEBUF_DECL_RW_4xN(v4,  v4,  4)
BYTEBUF_DECL_RW_4xN(v2i, v2i, 2)
...
BYTEBUF_DECL_RW_ARRAY(v3, v3)
BYTEBUF_DECL_RW_ARRAY(v4, v4)
BYTEBUF_DECL_RW_ARRAY(v2i, v2i)

// WARNING: uses tlscratch()!
M_INLINE int bytebuf_read_str(
    bytebuf_t *b,
    allocator_t *allocator,
    char **out) {
    DYNLIST(char) str = dynlist_create(char, tlscratch());
    char ch;
    int res, total = 0;
