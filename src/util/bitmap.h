...
#ifdef M_BITS_64
    // enfore 16-byte alignment of data when it immediately follows bitmap
    void *_pad;
#endif // ifdef M_BITS_64
} bitmap_t;
...
// debug only, check that bitmap is OK
#define BITMAP_CHECK(_pb) ({                                                   \
        ASSERT_DEBUG((_pb));                                                   \
        ASSERT_DEBUG((_pb)->size);                                             \
        ASSERT_DEBUG((_pb)->bits);                                             \
    })
...
M_INLINE void bitmap_copy(bitmap_t *dst, bitmap_t *src) {
    ASSERT_DEBUG(bitmap_valid(dst));
    ASSERT_DEBUG(bitmap_valid(src));
    ASSERT_DEBUG(dst->size >= src->size);
    ASSERT_DEBUG(src->size);
    ASSERT_DEBUG(dst->size);
    memcpy(dst->bits, src->bits, BITMAP_SIZE_TO_BYTES(src->size));
}

M_INLINE void bitmap_copy_n(bitmap_t *dst, bitmap_t *src, int size) {
    ASSERT_DEBUG(bitmap_valid(dst));
    ASSERT_DEBUG(bitmap_valid(src));
    ASSERT_DEBUG(src->size);
    ASSERT_DEBUG(dst->size);
    ASSERT_DEBUG(dst->size >= size);
    ASSERT_DEBUG(src->size >= size);
    memcpy(dst->bits, src->bits, BITMAP_SIZE_TO_BYTES(size));
}
...
// a |= b
M_INLINE void bitmap_or(bitmap_t *a, const bitmap_t *b) {
    const int n =
...
// a &= b
M_INLINE void bitmap_and(bitmap_t *a, const bitmap_t *b) {
    const int n =
        min(BITMAP_SIZE_TO_BYTES(a->size), BITMAP_SIZE_TO_BYTES(b->size));
...
M_INLINE int bitmap_find(const bitmap_t *b, int start, bool val) {
    ...
    if (M_UNLIKELY(start >= b->size)) { return -1; }
...
// if start == -1, the entire bitmap is searched
