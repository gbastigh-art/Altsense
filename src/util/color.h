...
M_INLINE v3 color_offset_with_hsv(v3 rgb, v3 hsv) {
    ...
    col_hsv.y = satf(col_hsv.y + hsv.y);
    col_hsv.z = satf(col_hsv.z + hsv.z);
    return color_hsv_to_rgb(col_hsv);
}

// lerp between two rgb colors in xyz space
M_INLINE v3 color_lerp_rgb(v3 a, v3 b, f32 t) {
    const v3 a_xyz = color_rgb_to_xyz(a), b_xyz = color_rgb_to_xyz(b);
    return color_xyz_to_rgb(v3_lerp(a_xyz, b_xyz, t));
}

// lerp between two rgba colors in xyz space
M_INLINE v4 color_lerp_rgba(v4 a, v4 b, f32 t) {
    return v4_of(color_lerp_rgb(v3_from(a), v3_from(b), t), lerp(a.a, b.a, t));
}
