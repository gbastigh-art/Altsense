...
/*
    ...
    Cmdline:
        sokol-shdc --input test/sim_blit.glsl --output test/sim_blit.glsl.h --slang metal_macos --format=sokol_impl -r

    Overview:
    =========
    Shader program: 'program':
        Get shader desc: sim_blit_program_shader_desc(sg_query_backend());
        Vertex shader: vs
            Attributes:
                ATTR_sim_blit_vs_position => 0
                ATTR_sim_blit_vs_texcoord0 => 1
            Uniform block 'vs_params':
                C struct: sim_blit_vs_params_t
                Bind slot: SLOT_sim_blit_vs_params => 0
        Fragment shader: fs
            Image 'tex':
                Image type: SG_IMAGETYPE_2D
                Sample type: SG_IMAGESAMPLETYPE_FLOAT
                Multisampled: false
                Bind slot: SLOT_sim_blit_tex => 0
            Sampler 'smp':
                Type: SG_SAMPLERTYPE_FILTERING
                Bind slot: SLOT_sim_blit_smp => 0
            Image Sampler Pair 'tex_smp':
                Image: tex
                Sampler: smp
*/
#if !defined(SOKOL_GFX_INCLUDED)
#error "Please include sokol_gfx.h before sim_blit.glsl.h"
#endif
...
#endif
#endif
const sg_shader_desc* sim_blit_program_shader_desc(sg_backend backend);
int sim_blit_program_attr_slot(const char* attr_name);
int sim_blit_program_image_slot(sg_shader_stage stage, const char* img_name);
int sim_blit_program_sampler_slot(sg_shader_stage stage, const char* smp_name);
int sim_blit_program_uniformblock_slot(sg_shader_stage stage, const char* ub_name);
size_t sim_blit_program_uniformblock_size(sg_shader_stage stage, const char* ub_name);
int sim_blit_program_storagebuffer_slot(sg_shader_stage stage, const char* sbuf_name);
int sim_blit_program_uniform_offset(sg_shader_stage stage, const char* ub_name, const char* u_name);
sg_shader_uniform_desc sim_blit_program_uniform_desc(sg_shader_stage stage, const char* ub_name, const char* u_name);
#define ATTR_sim_blit_vs_position (0)
#define ATTR_sim_blit_vs_texcoord0 (1)
#define SLOT_sim_blit_vs_params (0)
#define SLOT_sim_blit_tex (0)
#define SLOT_sim_blit_smp (0)
#pragma pack(push,1)
SOKOL_SHDC_ALIGN(16) typedef struct sim_blit_vs_params_t {
    m4 model;
    m4 view;
    m4 proj;
} sim_blit_vs_params_t;
#pragma pack(pop)
...
*/
static const uint8_t sim_blit_vs_source_metal_macos[595] = {
...
*/
static const uint8_t sim_blit_fs_source_metal_macos[391] = {
    0x23,0x69,0x6e,0x63,0x6c,0x75,0x64,0x65,0x20,0x3c,0x6d,0x65,0x74,0x61,0x6c,0x5f,
    0x73,0x74,0x64,0x6c,0x69,0x62,0x3e,0x0a,0x23,0x69,0x6e,0x63,0x6c,0x75,0x64,0x65,
    0x20,0x3c,0x73,0x69,0x6d,0x64,0x2f,0x73,0x69,0x6d,0x64,0x2e,0x68,0x3e,0x0a,0x0a,
    0x75,0x73,0x69,0x6e,0x67,0x20,0x6e,0x61,0x6d,0x65,0x73,0x70,0x61,0x63,0x65,0x20,
...
const sg_shader_desc* sim_blit_program_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_METAL_MACOS) {
        ...
        if (!valid) {
            valid = true;
            desc.vs.source = (const char*)sim_blit_vs_source_metal_macos;
            desc.vs.entry = "main0";
            desc.vs.uniform_blocks[0].size = 192;
            desc.vs.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.fs.source = (const char*)sim_blit_fs_source_metal_macos;
            desc.fs.entry = "main0";
            ...
            desc.label = "sim_blit_program_shader";
        }
        return &desc;
    }
    return 0;
...
size_t sim_blit_program_uniformblock_size(sg_shader_stage stage, const char* ub_name) {
    ...
    if (SG_SHADERSTAGE_VS == stage) {
        if (0 == strcmp(ub_name, "vs_params")) {
            return sizeof(sim_blit_vs_params_t);
        }
    }
    return 0;
}
